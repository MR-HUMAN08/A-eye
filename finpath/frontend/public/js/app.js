...
/**
 * Compute a composite Wealth Score (0–100) from:
 *  - Savings Rate (40%): surplus / income
 *  - Goal Feasibility (30%): is monthly surplus >= monthly savings needed?
 *  - Leakage Control (30%): how well spending stays within income-proportional benchmarks
 */
function computeWealthScore(profile, transactions) {
  const income = Number(profile.monthly_income || 0);
  if (income <= 0) return 0;

  const fixed = Number(profile.monthly_fixed_expenses || 0);
  const variable = transactions.reduce((sum, tx) => sum + Number(tx.amount || 0), 0);
  const surplus = Math.max(0, income - fixed - variable);

  // 1. Savings Rate component (40 pts max)
  // Use a curve: 20% savings rate = ~28 pts, 30% = ~34 pts, 50%+ = 40 pts
  const savingsRate = surplus / income; // 0.0 to 1.0
  const savingsScore = Math.min(40, Math.round(Math.sqrt(Math.min(1, savingsRate / 0.5)) * 40));

  // 2. Goal Feasibility component (30 pts max)
  const goalAmount = Number(profile.goal_amount || 0);
  const goalYears = Number(profile.goal_timeline_years || 0);
  let goalScore = 0;
  if (goalAmount > 0 && goalYears > 0) {
    const monthlyNeeded = goalAmount / (goalYears * 12);
    const feasibilityRatio = surplus / Math.max(1, monthlyNeeded); // >= 1 means on track
    goalScore = Math.min(30, Math.round(Math.min(1, feasibilityRatio) * 30));
  } else {
    goalScore = 15; // neutral when no goal set
  }

  // 3. Leakage Control component (30 pts max)
  // benchmark: each category should not exceed 8% of income (tighter control)
  const categoryBenchmark = income * 0.08;
  const categoryTotals = transactions.reduce((acc, tx) => {
    const key = tx.category || "Other";
    acc[key] = (acc[key] || 0) + Number(tx.amount || 0);
    return acc;
  }, {});
  const totalCategories = Math.max(1, Object.keys(categoryTotals).length);
  // Weight by how much each category exceeds benchmark (not just count)
  let totalExcess = 0;
  let totalSpend = 0;
  Object.values(categoryTotals).forEach(amt => {
    totalSpend += amt;
    if (amt > categoryBenchmark) totalExcess += (amt - categoryBenchmark);
  });
  const excessRatio = totalSpend > 0 ? totalExcess / totalSpend : 0;
  const leakageControlScore = Math.round((1 - Math.min(1, excessRatio * 2)) * 30);

  return Math.min(100, Math.max(0, savingsScore + goalScore + leakageControlScore));
}

/**
 * Compute leakage: returns the top overspending category and excess amount.
 * Benchmark per category = 12% of monthly income.
 */
function computeLeakage(profile, transactions) {
  const income = Number(profile.monthly_income || 0);
  const categoryBenchmark = income > 0 ? income * 0.08 : 5000; // 8% of income per category
  ...
  // Find the category with the highest EXCESS over benchmark (actual leak)
  const entries = Object.entries(categoryTotals)
    .map(([cat, amt]) => [cat, Number(amt), Math.max(0, Number(amt) - categoryBenchmark)])
    .sort((a, b) => b[2] - a[2]); // sort by excess descending
  const topEntry = entries[0] || ["N/A", 0, 0];
  const excess = topEntry[2];
  const amount = topEntry[1];
  const leakRatio = categoryBenchmark > 0 ? (amount / categoryBenchmark).toFixed(1) : "0.0";

  // Total leakage across all categories
  const totalExcess = entries.reduce((sum, e) => sum + e[2], 0);

  return { category: topEntry[0], amount, excess, leakRatio, totalExcess, benchmark: categoryBenchmark };
}
...
function updateDashboardMetrics(profile, transactions) {
  ...
  const wealthScore = computeWealthScore(profile, transactions);
  ...
  if (wealthSubtextEl) wealthSubtextEl.textContent = `Savings ${Math.round((monthlySurplus / Math.max(1, monthlyIncome)) * 100)}% · Goal · Leakage composite`;

  // Leakage Alert
  const leakage = computeLeakage(profile, transactions);
  const leakageMainEl = document.getElementById("leakage-main");
  const leakageSubEl = document.getElementById("leakage-sub");
  if (leakageMainEl) {
    if (leakage.totalExcess > 0) {
      leakageMainEl.textContent = `${currency(leakage.totalExcess)} total leakage`;
    } else {
      leakageMainEl.textContent = `No leakage detected`;
    }
  }
  if (leakageSubEl) {
    if (leakage.excess > 0) {
      leakageSubEl.textContent = `Top: ${leakage.category} at ${currency(leakage.amount)} (${leakage.leakRatio}x limit) · ${currency(leakage.excess)} over budget`;
    } else {
      leakageSubEl.textContent = `All categories within ${currency(leakage.benchmark)} budget · Healthy spending`;
    }
  }

  // Goal Progress: feasibility-based — can you reach your goal at current surplus rate?
  const goalAmount = Number(profile?.goal_amount || 0);
  const goalYearsMetric = Number(profile?.goal_timeline_years || 0);
  const totalMonths = goalYearsMetric > 0 ? goalYearsMetric * 12 : 0;
  // How much you'd save over the full goal period at current surplus
  const projectedSavings = monthlySurplusVal * totalMonths;
  // Feasibility %: projected savings / goal amount
  const goalProgress = goalAmount > 0 && totalMonths > 0
    ? Math.min(100, Math.round((projectedSavings / goalAmount) * 100))
    : 0;
  const monthsToGoal = monthlySurplusVal > 0 && goalAmount > 0
    ? Math.ceil(goalAmount / monthlySurplusVal)
    : 0;
  ...
  if (goalProgressText) {
    if (goalProgress >= 100) {
      goalProgressText.textContent = `${currency(monthlySurplusVal)}/month surplus · On track! Goal achievable in ${monthsToGoal} months`;
    } else if (goalAmount > 0) {
      goalProgressText.textContent = `${currency(monthlySurplusVal)}/month surplus · Need ${currency(Math.ceil(goalAmount / totalMonths))}/month to reach goal`;
    } else {
      goalProgressText.textContent = `Set goal to track progress`;
    }
  }
...
async function initDashboard() {
  ...
  if (form) {
    ...
    form.onsubmit = async (event) => {
      ...
      const updatedProfile = {
        ...
        existing_savings: 0, // actual accumulated savings — user hasn't specified
      ...
      // existing_savings stays 0 — it represents actual accumulated savings, not monthly surplus
      // Monthly surplus is computed separately in metric calculations
  ...
  const monthlyIncome = Number(profile.monthly_income || 0);
  // Use actual variable spend from transactions (NOT a hardcoded fallback)
  const monthlyVariable = transactions.length > 0
    ? transactions.reduce((sum, tx) => sum + Number(tx.amount || 0), 0)
    : Number(analysis?.monthly_variable_spend || 0);
  const monthlyFixed = Number(profile.monthly_fixed_expenses || 0);
  const monthlySurplus = Math.max(0, monthlyIncome - monthlyVariable - monthlyFixed);

  // Composite Wealth Score: savings rate + goal feasibility + leakage control
  const wealthScore = computeWealthScore(profile, transactions);
  ...
  document.getElementById("wealth-subtext").textContent = `Savings ${Math.round((monthlySurplus / Math.max(1, monthlyIncome)) * 100)}% · Goal · Leakage composite`;

  // Goal Progress: feasibility-based — projected savings over goal period vs goal amount
  const goalAmount = Number(profile?.goal_amount || 0);
  const goalYearsDash = Number(profile?.goal_timeline_years || 0);
  const totalMonthsDash = goalYearsDash > 0 ? goalYearsDash * 12 : 0;
  const projectedSavingsDash = monthlySurplus * totalMonthsDash;
  const progress = goalAmount > 0 && totalMonthsDash > 0
    ? Math.min(100, Math.round((projectedSavingsDash / goalAmount) * 100))
    : 0;
  const monthsToGoalDash = monthlySurplus > 0 && goalAmount > 0
    ? Math.ceil(goalAmount / monthlySurplus)
    : 0;
  if (progress >= 100) {
    document.getElementById("goal-progress-text").textContent = `${currency(monthlySurplus)}/month surplus · On track! Goal achievable in ${monthsToGoalDash} months`;
  } else if (goalAmount > 0 && totalMonthsDash > 0) {
    document.getElementById("goal-progress-text").textContent = `${currency(monthlySurplus)}/month surplus · Need ${currency(Math.ceil(goalAmount / totalMonthsDash))}/month to reach goal`;
  } else {
    document.getElementById("goal-progress-text").textContent = `Set goal to track progress`;
  }
  ...
  const countdownDays = monthsToGoalDash > 0 ? Math.round(monthsToGoalDash * 30) : (goalYearsDash > 0 ? Math.round(goalYearsDash * 365) : 0);
  document.getElementById("goal-countdown").textContent = countdownDays > 0 ? `${countdownDays} days remaining` : "--";

  // Leakage Alert: income-proportional benchmark (8% per category)
  const leakage = computeLeakage(profile, transactions);
  if (leakage.totalExcess > 0) {
    document.getElementById("leakage-main").textContent = `${currency(leakage.totalExcess)} total leakage`;
  } else {
    document.getElementById("leakage-main").textContent = `No leakage detected`;
  }
  if (leakage.excess > 0) {
    document.getElementById("leakage-sub").textContent = `Top: ${leakage.category} at ${currency(leakage.amount)} (${leakage.leakRatio}x limit) · ${currency(leakage.excess)} over budget`;
  } else {
    document.getElementById("leakage-sub").textContent = `All categories within ${currency(leakage.benchmark)} budget · Healthy spending`;
  }
